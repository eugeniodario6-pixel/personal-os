  ...
  return (
    <div style={{ fontFamily: 'var(--font-mono)' }}>
      <div style={{ padding: '1rem', borderBottom: '2px solid var(--border-strong)' }}>
        ...
        <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 510, color: 'var(--text)' }}>WHAT DID YOU DO?</h1>
      </div>
      {ITEMS.map((item) => (
        <button key={item.label} onClick={() => router.push(item.path)}
          ...
          <div>
            <p style={{ margin: '0 0 0.25rem', fontSize: '1.125rem', fontWeight: 510, color: 'var(--text)', letterSpacing: '0.01em' }}>{item.label}</p>
