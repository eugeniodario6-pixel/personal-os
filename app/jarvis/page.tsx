...
function VoicePicker({ voice, onSelect, onClose }: { voice: VoiceKey; onSelect: (v: VoiceKey) => void; onClose: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#0D0D0D', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 480, padding: '28px 20px 52px', border: 'none', borderBottom: 'none' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {VOICE_OPTIONS.map(v => (
            <button key={v.key} onClick={() => { onSelect(v.key); onClose(); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: 14, background: voice === v.key ? 'rgba(218,255,1,0.08)' : 'var(--color-carbon)', border: voice === v.key ? '1px solid rgba(218,255,1,0.35)' : 'none', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '0', overflow: 'hidden' }}>
      ...
      <div style={{ width: '100%', padding: '52px 24px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <button onClick={() => { unlockAudio(); setShowVoicePicker(true); }} style={{ background: 'var(--color-carbon)', border: 'none', borderRadius: 99, padding: '7px 14px', fontSize: '0.55rem', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontFamily: 'var(--font-mono)', WebkitTapHighlightColor: 'transparent' }}>
